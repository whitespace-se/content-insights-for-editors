...
class Dashboard {
              ...
              admin_url(
                add_query_arg(
                  array(
                    'post_id' => $value->ID,
                    'has-comment' => 'yes',
                    'after_date' => get_post_modified_time(
                      'U',
                      true,
                      $value->ID
                    ),
  ...
  public function renderCustomerResponses(
    $take = 10,
    $userID = false,
    $desc = true
  ) {
    $pages = PostQuery::getPostWithCustomerFeedbackResponses(
      $take,
      $desc,
      $userID
    );
